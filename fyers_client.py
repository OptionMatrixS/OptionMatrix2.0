...
Token from secrets.toml, symbol building, quotes, candles, Black-Scholes Greeks
"""
...
# ── Fyers Token (from secrets.toml) ──────────────────────────────────────────
# Generate your access token locally each morning using the Fyers TOTP script,
# then paste it into Streamlit Cloud secrets (or .streamlit/secrets.toml):
#
#   FYERS_ACCESS_TOKEN = "A0B798UIQA-100:eyJ...."
#
# The token format is: "APP_ID-APP_TYPE:access_token"


def is_fyers_connected():
    """Check if Fyers API is connected (token available in session)."""
    return "_fyers_token" in st.session_state and st.session_state["_fyers_token"]


def auto_connect_fyers():
    """
    Load Fyers token from st.secrets['FYERS_ACCESS_TOKEN'].
    ...
    try:
        ...
        if not token:
            raise ValueError("FYERS_ACCESS_TOKEN is empty in secrets.")
...
def render_fyers_login():
    """
    Render the Fyers connection status in the sidebar.
    Token is read from secrets.toml — no browser login needed.
    ...
    if success:
        ...
        st.sidebar.error(f"❌ Not connected: {error}")
        st.sidebar.markdown(
            "**To fix:**\n"
            "1. Run the token generator script locally\n"
            "2. Copy the access token\n"
            "3. Paste it in Streamlit Cloud **Secrets** as:\n\n"
            '```\nFYERS_ACCESS_TOKEN = "A0B798UIQA-100:ey..."\n```\n'
            "4. Click **🔄 Reconnect** to reload"
        )

        if st.sidebar.button("🔄 Retry", key="_fyers_retry_btn"):
...
def get_fyers_client():
    ...
    Token is read from secrets.toml.
    """
    ...
    if not token:
        ...
        if not token:
            raise RuntimeError(
                "Fyers API not connected. Set FYERS_ACCESS_TOKEN in secrets."
            )

    # Extract client_id from token (format: "APP_ID-APP_TYPE:access_token")
    client_id = token.split(":")[0] if ":" in token else token
...
def refresh_token():
    """Force token reload from secrets (clears cached token and client)."""
    _SS = st.session_state
    _SS.pop("_fc", None)
